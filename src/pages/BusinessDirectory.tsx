...
const BusinessDirectory = () => {
  ...
  const { 
    ...
    error, 
    refetch 
  } = useBusinesses(selectedCategory === 'all' ? undefined : selectedCategory)

  // Only use search when there's a non-empty search term
  const hasSearchTerm = debouncedSearchTerm.length > 0
  
  const { 
    data: searchResults, 
    loading: searchLoading 
  } = useSearchBusinesses(debouncedSearchTerm)
  ...
  // Keep previous results visible while loading new search
  const displayBusinesses = hasSearchTerm 
    ? (searchResults || []) 
    : (businesses || [])
  ...
  // If we have search results, use them directly (already filtered by API)
  // Otherwise, apply local filtering for the search term on the category-filtered results
  const filteredBusinesses = hasSearchTerm 
    ? displayBusinesses 
    : displayBusinesses.filter((business: Business) => {
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            <h3 className="text-lg font-medium text-red-800 mb-2">Error Loading Businesses</h3>
              ...
              <span className="text-gray-600">
                {hasSearchTerm && searchLoading ? (
                  ...
                  <>
                    ...
                    {hasSearchTerm && ` for "${debouncedSearchTerm}"`}
                  </>
                )}
              </span>
            </div>
